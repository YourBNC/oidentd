...
force_request:
	TOK_FORCE TOK_REQUEST TOK_STRING {
		cur_cap->caps = CAP_REQUEST;
		cur_cap->action = ACTION_FORCE;
		cur_cap->request_data = xrealloc(cur_cap->request_data, ++cur_cap->num_requests * sizeof(u_char *));
		cur_cap->request_data[cur_cap->num_requests - 1] = $3;
	}
|
	force_request TOK_STRING {
		if (cur_cap->num_requests < MAX_RANDOM_REPLIES) {
			cur_cap->request_data = xrealloc(cur_cap->request_data, ++cur_cap->num_requests * sizeof(u_char *));
			cur_cap->request_data[cur_cap->num_requests - 1] = $2;
		}
	}
...
user_request:
	TOK_REQUEST TOK_STRING {
		cur_cap->caps = CAP_REQUEST;
		cur_cap->request_data = xrealloc(cur_cap->request_data, ++cur_cap->num_requests * sizeof(u_char *));
		cur_cap->request_data[cur_cap->num_requests - 1] = $2;
	}
|
	user_request TOK_STRING {
		if (cur_cap->num_requests < MAX_RANDOM_REPLIES) {
			cur_cap->request_data = xrealloc(cur_cap->request_data, ++cur_cap->num_requests * sizeof(u_char *));
			cur_cap->request_data[cur_cap->num_requests - 1] = $2;
		}
